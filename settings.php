...
$backendService = $appContainer->query('OCA\Files_External_MOE\Service\BackendService');
$globalStoragesService = $appContainer->query('OCA\Files_External_MOE\Service\GlobalStoragesService');

OCP\Util::addScript('files_external_moe', 'settings');
OCP\Util::addStyle('files_external_moe', 'settings');

OCP\Util::addScript('files_external_moe', 'addmountpoint');
...
foreach ($backends as $backend) {
	if ($backend->getCustomJs()) {
		\OCP\Util::addScript('files_external_moe', $backend->getCustomJs());
	}
}
foreach ($authMechanisms as $authMechanism) {
	if ($authMechanism->getCustomJs()) {
		\OCP\Util::addScript('files_external_moe', $authMechanism->getCustomJs());
	}
}
...
$tmpl = new OCP\Template('files_external_moe', 'settings');
