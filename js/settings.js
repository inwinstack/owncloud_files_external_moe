...
var MOUNT_OPTIONS_DROPDOWN_TEMPLATE =
	...
	'		<label for="mountOptionsEncrypt">{{t "files_external_moe" "Enable encryption"}}</label>' +
	'	</div>' +
	'	<div class="optionRow">' +
	'		<input id="mountOptionsPreviews" name="previews" type="checkbox" value="true" checked="checked"/>' +
	'		<label for="mountOptionsPreviews">{{t "files_external_moe" "Enable previews"}}</label>' +
	'	</div>' +
	'	<div class="optionRow">' +
	'		<label for="mountOptionsFilesystemCheck">{{t "files_external_moe" "Check for changes"}}</label>' +
	'		<select id="mountOptionsFilesystemCheck" name="filesystem_check_changes" data-type="int">' +
	'			<option value="0">{{t "files_external_moe" "Never"}}</option>' +
	'			<option value="1" selected="selected">{{t "files_external_moe" "Once every direct access"}}</option>' +
	'			<option value="2">{{t "files_external_moe" "Every time the filesystem is used"}}</option>' +
	'		</select>' +
	'	</div>' +
	'</div>';
	...
	t("files_external_moe", "Enable encryption")
	t("files_external_moe", "Enable previews")
	t("files_external_moe", "Check for changes")
	t("files_external_moe", "Never")
	t("files_external_moe", "Once every direct access")
	t("files_external_moe", "Every time the filesystem is used")
	 */
...
function addSelect2 ($elements, userListLimit) {
	...
	$elements.select2({
		placeholder: t('files_external_moe', 'All users. Type to select user or group.'),
		allowClear: true,
		multiple: true,
		//minimumInputLength: 1,
		ajax: {
			url: OC.generateUrl('apps/files_external_moe/applicable'),
			dataType: 'json',
			quietMillis: 100,
		...
		formatSelection: function (element) {
			if (element.type === 'group') {
				return '<span title="'+escapeHTML(element.name)+'" class="group">'+escapeHTML(element.displayname+' '+t('files_external_moe', '(group)'))+'</span>';
			} else {
...
GlobalStorageConfig.prototype = _.extend({}, StorageConfig.prototype,
	...
	_url: 'apps/files_external_moe/globalstorages',
...
UserStorageConfig.prototype = _.extend({}, StorageConfig.prototype,
	...
	_url: 'apps/files_external_moe/userstorages'
});
...
$(document).ready(function() {
	var enabled = $('#files_external_moe').attr('data-encryption-enabled');
	...
	$('#sslCertificate').on('click', 'td.remove>img', function() {
		...
		$.post(OC.filePath('files_external_moe', 'ajax', 'removeRootCertificate.php'), {cert: $tr.attr('id')});
		$tr.remove();
		return true;
	});
	...
	$allowUserMounting.bind('change', function() {
		...
		if (this.checked) {
			OC.AppConfig.setValue('files_external_moe', 'allow_user_mounting', 'yes');
			...
			OC.AppConfig.setValue('files_external_moe', 'allow_user_mounting', 'no');
			$('#userMountingBackends').addClass('hidden');
		}
		OC.msg.finishedSaving('#userMountingMsg', {status: 'success', data: {message: t('files_external_moe', 'Saved')}});
	});
	...
	$('input[name="allowUserMountingBackends\\[\\]"]').bind('change', function() {
		...
		OC.AppConfig.setValue('files_external_moe', 'user_mounting_backends', userMountingBackends.join());
		OC.msg.finishedSaving('#userMountingMsg', {status: 'success', data: {message: t('files_external_moe', 'Saved')}});
