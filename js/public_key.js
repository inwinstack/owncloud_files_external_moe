$(document).ready(function() {
	...
	function setupTableRow(tr, config) {
		$(config).append($(document.createElement('input'))
			...
			.attr('type', 'button')
			.attr('value', t('files_external_moe', 'Generate keys'))
	...
	function generateKeys(tr) {
		...
		$.post(OC.filePath('files_external_moe', 'ajax', 'public_key.php'), {}, function(result) {
			if (result && result.status === 'success') {
				...
				OC.dialogs.alert(result.data.message, t('files_external_moe', 'Error generating key pair') );
			}
		});
	}
});
