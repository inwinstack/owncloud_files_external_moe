$(document).ready(function() {
	...
	OCA.External.Settings.mountConfig.whenSelectAuthMechanism(function($tr, authMechanism, scheme) {
		if (authMechanism === 'oauth1::oauth1') {
			...
			config.append($(document.createElement('input'))
				...
				.attr('type', 'button')
				.attr('value', t('files_external_moe', 'Grant access'))
			...
			if ($(configured).val() == 'true') {
				...
				$tr.find('.configuration').append('<span id="access" style="padding-left:0.5em;">'+t('files_external_moe', 'Access granted')+'</span>');
			} else {
				...
				if (app_key != '' && app_secret != '') {
					...
					if (pos != -1 && window.location.search.substr(pos, $(token).val().length) == $(token).val()) {
						...
						$.post(OC.filePath('files_external_moe', 'ajax', 'oauth1.php'), { step: 2, app_key: app_key, app_secret: app_secret, request_token: $(token).val(), request_token_secret: $(token_secret).val() }, function(result) {
							if (result && result.status == 'success') {
								...
								OC.dialogs.alert(result.data.message, t('files_external_moe', 'Error configuring OAuth1'));
							}
						});
					}
				}
	...
	$('#externalStorage').on('click', '[name="oauth1_grant"]', function(event) {
		...
		if (app_key != '' && app_secret != '') {
			...
			$.post(OC.filePath('files_external_moe', 'ajax', 'oauth1.php'), { step: 1, app_key: app_key, app_secret: app_secret, callback: location.protocol + '//' + location.host + location.pathname }, function(result) {
				if (result && result.status == 'success') {
			...
			OC.dialogs.alert(
				t('files_external_moe', 'Please provide a valid app key and secret.'),
				t('files_external_moe', 'Error configuring OAuth1')
			);
		}
	});
