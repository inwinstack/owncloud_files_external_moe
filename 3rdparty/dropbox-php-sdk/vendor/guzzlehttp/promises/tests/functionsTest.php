...
use DGuzzleHttp\Promise as P;
use DGuzzleHttp\Promise\FulfilledPromise;
use DGuzzleHttp\Promise\Promise;
use DGuzzleHttp\Promise\RejectedPromise;

class FunctionsTest extends \PHPUnit_Framework_TestCase
{
    public function testCreatesPromiseForValue()
    {
        $p = \DGuzzleHttp\Promise\promise_for('foo');
        $this->assertInstanceOf('DGuzzleHttp\Promise\FulfilledPromise', $p);
    }

    public function testReturnsPromiseForPromise()
    {
        $p = new Promise();
        $this->assertSame($p, \DGuzzleHttp\Promise\promise_for($p));
    }

    public function testReturnsPromiseForThennable()
    {
        $p = new Thennable();
        $wrapped = \DGuzzleHttp\Promise\promise_for($p);
        $this->assertNotSame($p, $wrapped);
        $this->assertInstanceOf('DGuzzleHttp\Promise\PromiseInterface', $wrapped);
        $p->resolve('foo');
        P\queue()->run();
    ...
    {
        $p = \DGuzzleHttp\Promise\rejection_for('fail');
        $this->assertInstanceOf('DGuzzleHttp\Promise\RejectedPromise', $p);
    ...
    {
        $a = new Promise();
        $b = \DGuzzleHttp\Promise\rejection_for($a);
        $this->assertSame($a, $b);
    }
    ...
    public function testWaitsOnAllPromisesIntoArray()
    {
        ...
        $results = \DGuzzleHttp\Promise\inspect_all([$a, $b, $c]);
        $this->assertEquals([
    ...
    public function testAllAggregatesSortedArray()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \DGuzzleHttp\Promise\all([$a, $b, $c]);
        $b->resolve('b');
        $a->resolve('a');
        $c->resolve('c');
        $d->then(
    ...
    public function testSomeAggregatesSortedArrayWithMax()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \DGuzzleHttp\Promise\some(2, [$a, $b, $c]);
        $b->resolve('b');
        $c->resolve('c');
        $a->resolve('a');
    ...
    public function testSomeRejectsWhenTooManyRejections()
    {
        $a = new Promise();
        $b = new Promise();
        $d = \DGuzzleHttp\Promise\some(2, [$a, $b]);
        $a->reject('bad');
        $b->resolve('good');
        P\queue()->run();
        ...
        P\queue()->run();
        $this->assertInstanceOf('DGuzzleHttp\Promise\AggregateException', $called);
    ...
    public function testThrowsIfResolvedWithoutCountTotalResults()
    {
        $a = new Promise();
        $b = new Promise();
        $d = \DGuzzleHttp\Promise\some(3, [$a, $b]);
        $a->resolve('a');
        $b->resolve('b');
        $d->wait();
    }
    ...
    public function testSettleFulfillsWithFulfilledAndRejected()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \DGuzzleHttp\Promise\settle([$a, $b, $c]);
        $b->resolve('b');
        $c->resolve('c');
        $a->reject('a');
        P\queue()->run();
    ...
    {
        $p = new Promise();
        $aggregate = \DGuzzleHttp\Promise\each_limit($p, 2);
        $p->resolve('a');
        P\queue()->run();
    ...
    public function testEachLimitAllRejectsOnFailure()
    {
        ...
        $aggregate = \DGuzzleHttp\Promise\each_limit_all($p, 2);
        P\queue()->run();
        $this->assertEquals(P\PromiseInterface::REJECTED, $aggregate->getState());
        $result = \DGuzzleHttp\Promise\inspect($aggregate);
    ...
    public function testIterForReturnsIterator()
    {
        ...
        $this->assertSame($iter, \DGuzzleHttp\Promise\iter_for($iter));
    }
    ...
    {
        $this->assertInstanceOf('DGuzzleHttp\Promise\TaskQueue', P\queue());
    ...
    public function testCanRejectFromRejectionCallback()
    {
        ...
        P\queue()->run();
        $this->assertInstanceOf('DGuzzleHttp\Promise\RejectionException', $result);
    ...
    public function testCoroutineOtherwiseIntegrationTest()
    {
        ...
        $this->assertInstanceOf('DGuzzleHttp\Promise\RejectionException', $reason->getPrevious());
    }
}
