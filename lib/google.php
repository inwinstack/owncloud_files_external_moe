...
class Google extends \OC\Files\Storage\Common {
	...
	private function onDuplicateFileDetected($path) {
		...
		$user = $about->getName();
		\OCP\Util::writeLog('files_external_moe',
		...
		if (isset(self::$tempFiles[$tmpFile])) {
			...
			if ($parentFolder) {
				...
				$mimetype = \OC::$server->getMimeTypeDetector()->detect($tmpFile);
                                if ($mimetype == 'inode/x-empty' && filesize($tmpFile) === 0){
                                    $mimetype='text/plain';
                                }
	...
	public function hasUpdated($path, $time) {
		...
		if ($this->is_file($path)) {
			...
			if ($folder) {
				...
				$startChangeId = $appConfig->getValue('files_external_moe', $this->getId().'cId');
				$params = array(
					'includeDeleted' => true,
				...
				$appConfig->setValue('files_external_moe', $this->getId().'cId', $largestChangeId);
				return $result;
			}
		}
		return false;
