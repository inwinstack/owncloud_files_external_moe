...
class EtagPropagator {
	...
	public function propagateDirtyMountPoints($time = null) {
		...
		foreach ($mountPoints as $mountPoint) {
			...
			$this->config->setUserValue($this->user->getUID(), 'files_external_moe', $mountPoint, $time);
		}
	...
	protected function getDirtyMountPoints() {
		$dirty = array();
		$mountPoints = $this->config->getAppKeys('files_external_moe');
		foreach ($mountPoints as $mountPoint) {
			if (substr($mountPoint, 0, 1) === '/') {
				$updateTime = $this->config->getAppValue('files_external_moe', $mountPoint);
				$userTime = $this->config->getUserValue($this->user->getUID(), 'files_external_moe', $mountPoint);
	...
	protected function markDirty($mountPoint, $time = null) {
		...
		$this->config->setAppValue('files_external_moe', $mountPoint, $time);
	}
