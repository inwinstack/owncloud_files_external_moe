...
use \OCA\Files_External_MOE\Appinfo\Application;
use \OCA\Files_External_MOE\Lib\BackendConfig;
use \OCA\Files_External_MOE\Service\BackendService;
use \OCA\Files_External_MOE\Lib\Backend\LegacyBackend;
...
class OC_Mount_Config {
	 ...
	 * @return bool
	 * @deprecated 8.2.0 use \OCA\Files_External_MOE\Service\BackendService::registerBackend()
	 */
	public static function registerBackend($class, $definition) {
		$backendService = self::$app->getContainer()->query('OCA\Files_External_MOE\Service\BackendService');
		$auth = self::$app->getContainer()->query('OCA\Files_External_MOE\Lib\Auth\Builtin');
		...
		if ($data['user']) {
			...
			if (!$user) {
				\OC::$server->getLogger()->warning(
					...
					['app' => 'files_external_moe']
				);
				return;
			}
			$userView = new \OC\Files\View('/' . $user->getUID() . '/files');
			$changePropagator = new \OC\Files\Cache\ChangePropagator($userView);
			$etagPropagator = new \OCA\Files_External_MOE\EtagPropagator($user, $changePropagator, \OC::$server->getConfig());
	...
	public static function getAbsoluteMountPoints($uid) {
		...
		$userGlobalStoragesService = self::$app->getContainer()->query('OCA\Files_External_MOE\Service\UserGlobalStoragesService');
		$userStoragesService = self::$app->getContainer()->query('OCA\Files_External_MOE\Service\UserStoragesService');
	...
	public static function getSystemMountPoints() {
		$mountPoints = [];
		$service = self::$app->getContainer()->query('OCA\Files_External_MOE\Service\GlobalStoragesService');
	...
	public static function getPersonalMountPoints() {
		$mountPoints = [];
		$service = self::$app->getContainer()->query('OCA\Files_External_MOE\Service\UserStoragesService');
	...
	public static function getBackendStatus($class, $options, $isPersonal) {
		...
		if (class_exists($class)) {
			try {
				...
				\OCP\Util::logException('files_external_moe', $exception);
			}
		}
